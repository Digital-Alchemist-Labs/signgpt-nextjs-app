...
const HandTracker = forwardRef<HandTrackerRef, HandTrackerProps>(
  ({ onKeypointsDetected, isRecording, showKeypoints = true }, ref) => {
    ...
    const loadMediaPipe = async () => {
      ...
      try {
        ...
        if (
          ...
          (window as unknown as Record<string, unknown>).Holistic
        ) {
          console.log("CDN에서 로드된 MediaPipe Holistic 사용");
          mediaPipeRef.current = {
            Holistic: (window as unknown as Record<string, unknown>).Holistic,
            drawConnectors: (window as unknown as Record<string, unknown>)
              .drawConnectors,
            drawLandmarks: (window as unknown as Record<string, unknown>)
              .drawLandmarks,
            HAND_CONNECTIONS: (window as unknown as Record<string, unknown>)
              .HAND_CONNECTIONS,
            POSE_CONNECTIONS: (window as unknown as Record<string, unknown>)
              .POSE_CONNECTIONS,
            FACEMESH_CONTOURS: (window as unknown as Record<string, unknown>)
              .FACEMESH_CONTOURS,
          };
    ...
    const initializeCamera = useCallback(async () => {
      ...
      if (
        typedMediaPipe.Holistic &&
        typeof typedMediaPipe.Holistic === "function"
      ) {
        console.log("MediaPipe Holistic 모델 초기화 중...");
        holistic = new (typedMediaPipe.Holistic as new (
          config: unknown
        ) => unknown)({
        ...
        (
          ...
          if (showKeypoints) {
            ...
            if (
              ...
              (
                typedMediaPipe.drawConnectors as (
                  ctx: CanvasRenderingContext2D,
                  landmarks: unknown,
                  connections: unknown,
                  options?: unknown
                ) => void
              )(ctx, results.faceLandmarks, typedMediaPipe.FACEMESH_CONTOURS, {
                color: "#C0C0C070",
                lineWidth: 1,
              });
            }
            ...
            if (
              ...
              )(ctx, results.poseLandmarks, typedMediaPipe.POSE_CONNECTIONS, {
                color: "#00CEFF",
                lineWidth: 2,
              });
              if (typedMediaPipe.drawLandmarks) {
                (
                  typedMediaPipe.drawLandmarks as (
                    ctx: CanvasRenderingContext2D,
                    landmarks: unknown,
                    options?: unknown
                  ) => void
            ...
            if (
              ...
              if (typedMediaPipe.drawLandmarks) {
                ...
                )(ctx, results.rightHandLandmarks, {
                  color: "#00FF00",
                  lineWidth: 1,
                  radius: 3,
                });
          ...
          if (results.leftHandLandmarks && results.rightHandLandmarks) {
            const leftHand = landmarkToList(
              results.leftHandLandmarks as Array<{
                x: number;
                y: number;
                z?: number;
              }>
            );
            const rightHand = landmarkToList(
              results.rightHandLandmarks as Array<{
                x: number;
                y: number;
                z?: number;
              }>
            ...
            // Collect keypoints in auto-recording mode
            if (isAutoRecording) {
    ...
    useEffect(() => {
      ...
      const initializeMediaPipe = async () => {
        try {
          ...
          // Start auto-recording mode after camera initialization
          console.log(
            "Camera initialization complete - starting auto-recording mode"
          );
          ...
          if ((err as Error).name === "NotAllowedError") {
            ...
            errorMessage =
              "Camera not found. Please check if camera is connected.";
          } else if ((err as Error).name === "NotReadableError") {
            errorMessage = "Camera is being used by another application.";
          } else if ((err as Error).name === "OverconstrainedError") {
            errorMessage = "Requested camera settings are not supported.";
    ...
    const retryCamera = async () => {
      console.log("Starting camera restart...");
      setIsRetrying(true);
      setError(null);
      ...
      setTimeout(async () => {
        try {
          ...
          let errorMessage = "Camera restart failed.";
          if ((err as Error).name === "NotAllowedError") {
            errorMessage =
              "Camera access denied. Please allow camera permission in browser settings.";
          } else if ((err as Error).name === "NotFoundError") {
      ...
      return (
        <div className="w-full h-80 bg-red-100 border border-red-300 rounded-lg flex items-center justify-center">
          <div className="text-red-600 text-center">
            ...
            >
              {isRetrying ? "Retrying..." : "Restart Camera"}
            </button>
          </div>
        </div>
      );
    ...
    return (
      <div className="relative w-full">
        ...
        <div className="absolute bottom-4 left-4 bg-black/50 text-white px-3 py-1 rounded-lg text-sm">
          Show your face and both hands on screen
        </div>
      </div>
    );
  }
