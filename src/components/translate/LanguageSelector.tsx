		...
		<div className="space-y-4">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				...
				<div className="space-y-2">
					<label className="text-sm font-medium">From (Spoken Language)</label>
						...
						{isSourceOpen && (
							<div className="absolute z-50 w-full mt-1 bg-background border rounded-md shadow-lg max-h-60 overflow-y-auto">
								{spokenLanguages.map((language) => (
									<button
										key={language.code}
										onClick={() => handleSourceLanguageChange(language.code)}
										className={`w-full text-left px-3 py-2 hover:bg-accent transition-colors flex items-center space-x-2 ${
											language.code === state.sourceLanguage ? "bg-accent" : ""
										}`}>
				...
				{/* Target Language */}
				<div className="space-y-2">
					<label className="text-sm font-medium">To (Sign Language)</label>
					<div className="relative">
						<button
						...
						{isTargetOpen && (
							<div className="absolute z-50 w-full mt-1 bg-background border rounded-md shadow-lg max-h-60 overflow-y-auto">
								{signedLanguages.map((language) => (
									<button
										key={language.code}
										onClick={() => handleTargetLanguageChange(language.code)}
										className={`w-full text-left px-3 py-2 hover:bg-accent transition-colors flex items-center space-x-2 ${
											language.code === state.targetLanguage ? "bg-accent" : ""
										}`}>
										<span className="text-lg">{language.flag}</span>
										<span>{language.name}</span>
									</button>
